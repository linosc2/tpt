 ...
 ///$('#loginButton').live('click',function(login){
   ...
   // var form = $('#loginForm');
    ...
    $(this).mouseup(function(menu) {
 	...
 	$('#logoPublic').live('click',function(){
      $('#popup').load('imagen.php',function(){
		  $('#popup').show('slow');
			$('.popup-overlay').fadeIn('slow');
			$('.popup-overlay').height($(window).height());
...
var encabezado = L.control({position:'topleft'});

		encabezado.onAdd = function (map) {

			var div = L.DomUtil.create('div', ' encabezado'),
			labels = [];
...
div.innerHTML = labels.push('<div id="loginContainer"><a href="javascript:void(0);" id="loginButton"><span>Login</span><em></em></a><div style="clear:both"></div><div id="loginBox"><form name ="loginForm" id="loginForm" method="POST" action="index.php"><fieldset id="body"><fieldset><label for="email">Email Address</label><input type="text" name="email" id="email" /></fieldset><fieldset><label for="password">Password</label><input type="password" name="password" id="password" /></fieldset><input type="submit" id="logins" value="Sign in" /></fieldset><span><a href="#">Forgot your password?</a></span></form></div></div>');
///div.innerHTML = labels.push('<div id="menuContainer"><a href="javascript:void(0);" id="upload_button">Upload</a><ul id="lista"></ul></div>');

<?php
} else {
?> 
div.innerHTML = labels.push('BIENVENIDO');
<?php
...
if (strlen($foto)== 0)
		{
?> 
div.innerHTML = labels.push('<img src="images/noDisponible.png" height="30" width="30" border="0"> <?php echo $nombre; ?>');
<?php
}else {
?> 
div.innerHTML = labels.push('<img src="logo/<?php echo $foto; ?>" height="30" width="30"> <?php echo $nombre; ?>');
<?php
		}
?> 

div.innerHTML = labels.push('<div id="menuContainer"><a href="javascript:void(0);" id="openmenu"><img src="images/icon-cog-shadow.png" height="20" width="20"/></a><div style="clear:both"></div><div id="menubox"><div id="menuForm"><fieldset id="body"><a href="javascript:void(0);" id="logoPublic" class="buttonmenu" ><span>agregar un logo</span></a><br /><br /><br /><a href="javascript:void(0);" id="passPublic" class="buttonmenu" ><span>cambiar password</span></a><br /><br /><br /><a href="logout.php" id="" class="buttonmenu" ><span>Salir</span></a></div></div>');
<?php
}
}
?>	
